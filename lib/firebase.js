	...
	ref.on('child_added', (snapshot, prevChildKey) => {
		...
		createDataSource(oauth2.getToken()).then((value) => {
			...
			if (dataSourceId == null) {
				logger.error('An unexpected error has been occurred. (data source is null)\n')
				return;
			}

			logger.info(`Add weight: ${weight} to the data source`);

			// データセットを追加
			createDataSets(weight).then(() => {
				logger.info('Successed.\n');	
			}).catch((error) => {
				logger.error('Failed to regist dataset to Google Fit.\n')
				logger.error(error);
			});
		}).catch((error) => {
			logger.error(error);
	...
	return new Promise((resolve, reject) => {
		request(options, (err, res, body) => {
			...
			if ('dataStreamId' in body) {
				...
				logger.info('Data source has been already created.')
				var message = body.error.message
				dataSourceId = message.match(/Data Source: (.*) already exists/)[1]
			}
			else {
				// それ以外の状況は想定していない．
				logger.info('An unexpected error has been occurred.')
			}
			return resolve(body)
		})
	});
